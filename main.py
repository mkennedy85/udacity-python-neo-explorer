...
The `query` subcommand searches for close approaches that match given criteria:
    ...
    $ python3 main.py query --start-date 2020-01-01 --max-distance 0.025
    $ python3 main.py query --start-date 2050-01-01 --min-velocity 50
    $ python3 main.py query --date 2020-03-14 --min-diameter 0.5 --hazardous
    $ python3 main.py query --start-date 2000-01-01 --not-hazardous
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        description=("""Explore past and future close approaches of near-Earth
        objects.""")
    )
    ...
    filters = query.add_argument_group('Filters',
                                       description=("Filter close approaches "
                                                    "by their attributes "
                                                    "or the attributes of "
                                                    "their NEOs."))
    filters.add_argument('-d', '--date', type=date_fromisoformat,
                         help=("Only return close approaches on the given "
                               "date, in YYYY-MM-DD format "
                               "(e.g. 2020-12-31)."))
    filters.add_argument('-s', '--start-date', type=date_fromisoformat,
                         help=("Only return close approaches on or after the "
    ...
    filters.add_argument('--min-distance', dest='distance_min', type=float,
                         help=("In astronomical units. Only return close "
                               "approaches that pass as far or farther away "
                               "from Earth as the given distance."))
    filters.add_argument('--max-distance', dest='distance_max', type=float,
                         help=("In astronomical units. Only return close "
                               "approaches that pass as near or nearer to "
    ...
    filters.add_argument('--min-velocity', dest='velocity_min', type=float,
                         help=("In kilometers per second. Only return close "
                               ...
                               "at approach is as fast or faster than the "
                               "given velocity."))
    ...
    filters.add_argument('--not-hazardous', dest='hazardous', default=None,
                         action='store_false',
                         help=("If specified, only return close approaches "
                               "of NEOs that are not potentially hazardous."))
    ...
    repl.add_argument('-a', '--aggressive', action='store_true',
                      help=("If specified, kill the session whenever a "
...
def inspect(database, pdes=None, name=None, verbose=False):
    ...
    This function fetches an NEO by designation or by name. If a matching NEO
    is found, information about the NEO is printed (additionally, information
    for all of the NEO's known close approaches is printed if `verbose=True`).
    ...
    :param database: The `NEODatabase` containing data on NEOs and their close
    approaches.
...
class NEOShell(cmd.Cmd):
    ...
    It wraps the `inspect` and `query` parsers to parse flags for those
    commands
    ...
    def __init__(self, database, inspect_parser, query_parser,
        ...
        :param aggressive: Whether to kill the session whenever a project file
        is changed.
    ...
    def do_query(self, arg):
        ...
        command line. For example, to query close approaches on
        January 1st, 2020:
        ...
        The number of results shown can be limited to a maximum number
        with `--limit`:
    ...
    def precmd(self, line):
        ...
        if changed:
            print(f"""The following file(s) have been modified since this """
